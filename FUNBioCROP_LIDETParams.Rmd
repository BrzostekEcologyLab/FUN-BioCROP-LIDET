---
title: "FUN-BioCROP with LIDET Parameters for Litter Decomposition"
...
This is the FUN-BioCROP model (https://github.com/BrzostekEcologyLab/FUN-BioCROP) updated to use new decomposition parameters in the litter compartment that were derived from a modified Monte Carlo analysis.

*This code is set up for ANNUALS (only harvest is different depending on annual v. perennial). To change for perennials, change harvest as indicated on line 371 below. 
...
# Initial C and N pool values for bulk, rhizosphere and litter layers (kg C or N/m2, from spinup)
...
input <- read.csv("input_UIEnergyFarm_CCS_2008-2100.csv", header = TRUE) # in kg C/m2
...
# litter specific parameters from LIDET reparameterization
# parameter values that were changed have the original FUN-BioCROP values commented next to them
params_litter <-data.frame(
  "Vmaxref_Fast" = 2.0, # 9.0 in original params (above)
  "Vmaxref_Slow" = 0.35, # 0.25 in original params (above)
  "Vmaxref_Necro"= 4.5,
  "Ea_Fast" = 5e3,
  "Ea_Slow" = 25e3, # 30e3 in original params (above)
  "Ea_Necro"= 3e3,
  "kC_Fast" = 0.007, # 0.01 in original params (above)
  "kC_Slow" = 0.005, # 0.01 in original params (above)
  "kC_Necro"= 0.007, # 0.01 in original params (above)
  "gas_diffusion_exp" = 0.6,
  "minMicrobeC" = 1e-3,
  "Tmic"= 0.25,
  "et" = 0.6,
  "eup_Fast" = 0.6,
  "eup_Slow"= 0.001, # 0.1 in original params (above)
  "eup_Necro" = 0.6,
  "tProtected" = 100.0,
  "frac_N_turnover_min" = 0.2,
  "protection_rate_Fast" = 0.7,
...
#subset every other row for export, because every other row is a different "site"
...
# first prepare a column of dates to cbind to tc dataframe below
date_col <- as.data.frame(1:(nrow(totalsoilC)+23)) # add # of leap days during run
colnames(date_col) <- c("cumday")
date_col$date <- as.Date(date_col$cumday, origin = as.Date("2007-12-31"))
...
data_export <- 
  ...
  mutate (date = tc$date, 
          ...
          whole_soil_ratio = protection_ratios$whole_soil_ratio,
...
```{r Export Output Data}


# Results are in kg C/m2

### Export file with all data used in analysis
write.csv(data_export, "FUN-BioCROP_LIDETParams_CCS_AllData.csv", row.names = FALSE)
...
ggplot() +
  ...
  labs(y = expression(Carbon~remaining~(kg~m^{-2})),
       ...
       subtitle = "UI Energy Farm, FUN CORPSE Bioenergy") #+
  #ylim(6.5, 9.5)+
  xlim(as.Date(c("2015-09-01", "2016-10-01")))
