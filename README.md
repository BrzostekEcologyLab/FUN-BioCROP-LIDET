...
​	d.   **input_UIEnergyFarm_CCS_2008-2100.csv**: This file has daily data to run FUN-BioCROP: 
...
1) Save the model code (“FUN-BioCROP_LIDETParams.Rmd”) and accompanything files (data streams and CORPSE function code) in the same folder. 
...
5. **Chunk 5: Load Parameters.** Loads all fixed parameters to run the model. This now includes the new LIDET-derived parameters for the litter compartment as well as the baseline parameters for the rhizosphere and bulk compartments.  Data frame with definitions of parameters is in the CORPSE function script “CORPSE Functions_Bioenergy.R”
